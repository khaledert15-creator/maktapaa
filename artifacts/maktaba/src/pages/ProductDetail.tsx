  ...
  return <div className="container mx-auto px-4 py-7 sm:py-10">
    ...
    <div className="grid gap-9 lg:grid-cols-[1fr_1fr] lg:gap-14">
      <section className="grid gap-3 sm:grid-cols-[84px_1fr]">
        ...
        <button onClick={() => selectedImage && setZoomOpen(true)} className="group relative order-1 mx-auto aspect-[3/4] w-full max-w-lg overflow-hidden rounded-3xl border bg-slate-50 sm:order-2" aria-label="تكبير صورة المنتج">{selectedImage ? <img src={selectedImage} alt={product.nameAr} width="640" height="850" className="h-full w-full object-contain transition duration-500 group-hover:scale-105" /> : <BookOpen className="m-auto h-full w-28 text-muted-foreground/25" />}<span className="absolute bottom-4 left-4 rounded-full bg-white/95 p-3 shadow"><Maximize2 className="h-5 w-5" /></span>{!!product.discountPercent && product.discountPercent > 0 && <Badge className="absolute right-4 top-4 bg-rose-600 px-3 py-1.5 text-white">خصم {product.discountPercent}%</Badge>}</button>
      </section>
