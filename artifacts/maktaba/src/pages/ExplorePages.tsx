...
export function PublishersPage() {
  ...
  return <Listing title="دور النشر" subtitle="تصفح كتب الناشر المفضل لديك" icon={Building2}>{isLoading ? <GridSkeleton short /> : data?.length ? <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{data.map(item => <Link key={item.id} href={`/catalog?publisherId=${item.id}`}><Card className="h-full rounded-2xl transition hover:-translate-y-1 hover:border-secondary hover:shadow-lg"><CardContent className="flex min-h-36 flex-col items-center justify-center gap-4 p-5 text-center">{item.logo ? <img src={item.logo} alt={item.nameAr} className="max-h-16 max-w-full object-contain" /> : <Building2 className="h-10 w-10 text-secondary" />}<strong>{item.nameAr}</strong></CardContent></Card></Link>)}</div> : <Empty text="لم تُضف دور نشر بعد" />}</Listing>;
}
export function CategoriesPage() {
  const { data, isLoading } = useListCategories();
  return <Listing title="تصنيفات المكتبة" subtitle="كل قسم يقودك للكتب المرتبطة به" icon={Library}>{isLoading ? <GridSkeleton short /> : data?.length ? <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">{data.map(item => <Link key={item.id} href={`/catalog?categoryId=${item.id}`}><Card className="h-full overflow-hidden rounded-2xl transition hover:-translate-y-1 hover:border-secondary hover:shadow-lg">{item.image && <img src={item.image} alt={item.nameAr} className="h-36 w-full object-cover" />}<CardContent className="p-5"><strong className="text-lg">{item.nameAr}</strong><p className="mt-1 text-sm text-muted-foreground">{item.productCount || 0} منتج</p></CardContent></Card></Link>)}</div> : <Empty text="لم تُضف تصنيفات بعد" />}</Listing>;
}
